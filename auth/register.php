...
if(isset($_POST['regist'])){
    ...
    $sql="insert into users(username, passsword, role) values(?,?,'User')";
    $stmtinsert=$db->prepare($sql);
    $result=$stmtinsert->execute([$username, $password, $role]);
    if($result){
        echo "<script>alert('Account successfully registered.')</script>";
        echo "<script>location.reload();</script>";
        exit();
    }else{
        echo "<script>alert('Tehere were errors while saving the data')</script>";
        echo "<script>location.reload();</script>";
        exit();
    }
}
